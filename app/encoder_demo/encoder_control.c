...
void EncoderEntry(void)
{
    ...
    while (Flag) {
        // 打印编码器的计数值
        //printf("encoderLeftACounter = %d, encoderLeftBCounter = %d\r\n", encoderLeftACounter, encoderLeftBCounter);
        if (rc == 1) {
        printf("A , forward, encoderLeftACounter = %d\r\n", encoderLeftACounter);
        } else if (rc == FLAG_BIT) {
            printf("B, backward, encoderLeftBCounter = %d\r\n", encoderLeftBCounter);
    ...
    // 取消GPIO12的中断功能
    IoTGpioUnregisterIsrFunc(IOT_IO_NAME_GPIO_12);
    // 输出Left Stop！
    printf("Left Stop!\n");
}
