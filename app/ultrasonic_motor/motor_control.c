...
#define IOT_DUTY            50
#define IOT_DUTY_MAX        100
#define IOT_DUTY_MIN        0
#define LOW_DUTY            20
#define HIGH_DUTY           80  
...
void Backword(void)
{
    // 实现左电机向后转动 The left motor rotates backward
    LeftWheelBackword();
    // 实现右电机向后转动 The right motor rotates backward
    RightWheelBackword();
}
void Stop(void)
{
    // 实现左电机停止转动 The left motor stops rotating
    LeftWheelStop();
    // 实现右电机停止转动 The right motor stops rotating
    RightWheelStop();
}
void Left(void)
{
    // 实现左电机向前转动 The left motor rotates backward
    IoTPwmStart(IOT_PWM_PORT_PWM2, IOT_DUTY, IOT_FREQ);
    // 实现右电机向前转动 The right motor rotates forward
    IoTPwmStart(IOT_PWM_PORT_PWM1, LOW_DUTY, IOT_FREQ);
}
void Right(void)
{
    // 实现左电机向前转动 Realize the forward rotation of the left motor
    IoTPwmStart(IOT_PWM_PORT_PWM2, LOW_DUTY, IOT_FREQ);
    // 实现右电机向前转动 The right motor rotates backward
    IoTPwmStart(IOT_PWM_PORT_PWM1, IOT_DUTY, IOT_FREQ);
}
